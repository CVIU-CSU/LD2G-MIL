...
class NFIRgMilHead(ClsHead):
    ...
    def __init__(self,
        ...
        self.linear1 = nn.Parameter(
            data=torch.FloatTensor(self.in_channels, 2))
    ...
    def simple_test(self, x):
        ...
        if Y_hat1 == 0 and Y_hat2 == 0:
            ...
            C = torch.tensor(
                [[(Y_logits1[0] + Y_logits2[0]) / 2, Y_logits1[1], Y_logits2[1]]])
    ...
    def forward_train(self, x, gt_label, **kwargs):
        ...
        losses = self.loss(Y_logits1, Y_hat1, Y_logits2,
                           Y_hat2, gt_label, **kwargs)
        return losses
    ...
    def loss(self, Y_logits1, Y_hat1, Y_logits2, Y_hat2, gt_label, **kwargs):
        ...
        losses = dict()
        # compute loss
        gt1 = torch.where(gt_label == 1, torch.ones_like(
            gt_label), torch.zeros_like(gt_label))
        gt2 = torch.where(gt_label == 2, torch.ones_like(
            gt_label), torch.zeros_like(gt_label))

...
class ConcatDataset(_ConcatDataset):
    ...
    def evaluate(self, results, **kwargs):
        index1 = len(self.datasets[0])
        index2 = len(self.datasets[1])
        eval_result = {}

        eval_result.update(
            add_prefix(self.datasets[0].evaluate(results[:index1], **kwargs), 'train')
        )

        eval_result.update(
            add_prefix(self.datasets[1].evaluate(results[index1:index1+index2], **kwargs), 'val')
        )
        
        if is_index_valid(self.datasets, 2):
            eval_result.update(
                add_prefix(self.datasets[2].evaluate(results[index1+index2:], **kwargs), 'test')
            )

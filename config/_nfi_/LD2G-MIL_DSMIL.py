...
data = dict(
    ...
    train=dict(
        type='NFI',
        classes=['NEG', 'RH', 'ROP'],
        data_prefix='/root/commonfile/guojie/nfi_feature/23layer/data_611/train',
        pipeline=[
            dict(type='LoadPtFromFile_vit', self_normalize=True),
            dict(type='ToTensor', keys=['gt_label']),
            dict(type='Collect', keys=['img', 'gt_label'])
    ...
    val=[
        ...
        dict(
            type='NFI',
            classes=['NEG', 'RH', 'ROP'],
            data_prefix='/root/commonfile/guojie/nfi_feature/23layer/data_611/val',
            pipeline=[
                dict(type='LoadPtFromFile_vit', self_normalize=True),
                dict(type='Collect', keys=['img'])
            ])
...
work_dir = 'work_dir/ablation/Components/DSMIL/ts14init_50ep_ce'
gpu_ids = range(0, 4)
